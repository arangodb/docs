...
For ArangoDB version before 3.0 look at the [old Compiling ArangoDB under Windows](compiling-windows.html).

## Solution

With ArangoDB 3.0 a complete cmake environment was introduced. This also streamlines the dependencies on windows.
We sugest to use [chocolatey.org](https://chocolatey.org/){:target="_blank"} to install most of the dependencies. For sure
...
[ansibles 2.0 winrm facility](http://docs.ansible.com/ansible/intro_windows.html){:target="_blank"}
...
Then we fetch the [OpenSSL](https://openssl.org){:target="_blank"} library via the nuget commandline client *(doesn't need Administrator privileges)*:
...
The documentation and unittests still require a [cygwin](https://www.cygwin.com/){:target="_blank"} environment. Here the hints how to get it properly installed:
