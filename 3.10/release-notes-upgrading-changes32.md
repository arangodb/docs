...
* JWT tokens issued by the built-in [JWT session storage](foxx-reference-sessions-storages-jwt.html) now correctly specify the `iat` and `exp` values in seconds rather than milliseconds as specified in the JSON Web Token standard.
