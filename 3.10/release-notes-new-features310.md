...
Some key generators can generate keys in ascending order, meaning that document
...
server deployments. The reason was that document keys could be generated by the 
DB server, but also by coordinators (of which there are normally multiple instances). 
...
ascending. 
ArangoDB version 3.10 changes this behavior so that collections with only a single 
