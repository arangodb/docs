...
When certificates that are being used in existing deployments are about to expire,
an automatic rotation of the certificates will be triggered. This means that the
certificate will be cloned and all deployments using it will then start using
the cloned certificate. 

Based on the type of certificate used, you may also need to install the new
certificate on your local machine. To prevent downtime, it is recommended to
