...
- Added endpoint for new version V2 of the metrics API:
  ...
  The old endpoint `GET /_admin/metrics` is still supported but is
  considered to be obsolete from 3.8 on (see [in the new features
  section](release-notes-new-features38.html#metrics) for reasons) and
  will be removed in a future version. In the new API V2, there are quite
  a lot more metrics than in previous versions and a lot have been
  renamed to follow Prometheus conventions. Here is a list of renamed
  metrics:
