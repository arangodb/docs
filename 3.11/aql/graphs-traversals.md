...
- `PRUNE` **condition** (AQL condition, *optional*):
  A condition, like in a `FILTER` statement, which is evaluated in every step of
  the traversal, as early as possible. The semantics of this condition are as follows:
  - If the condition evaluates to `false`, the traversal continues on the current path.
  - If the condition evaluates to `true`, the traversal does not continue on the
  ...
  the prune condition can contain an arbitrary number of `AND` and `OR`
  statements for complex expressions. You can use the variables emitted by the
  `FOR` operation in the prune condition, as well as all variables defined
  before the traversal.

  You can optionally assign the prune condition to a variable to use the
  evaluated result elsewhere in the query, like in a `FILTER` condition.
- `OPTIONS` **options** (object, *optional*): used to modify the execution of the
  ...
  - **parallelism** (number, *optional*): Optionally parallelize traversal
    ...
    execution.
    {% include hint-ee.md feature="Traversal parallelization" %}
...
to be checked during a traversal. It allows you to improve the query performance and reduce
...
This searches until it sees an edge having `theTruth == true`.
