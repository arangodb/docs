---
layout: default
description: Any operation triggered via ArangoDB's HTTP REST API is executed in the context of exactly one database
---
...
where `...` is the actual path to the accessed resource. In the example, the resource is
accessed in the context of the database `mydb`. Actual URLs in the context of `mydb` look
like this:
...
Non-NFC-normalized names are rejected by _arangod_.
