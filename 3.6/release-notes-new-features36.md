...
communication between the coordinator and database server is removed. This will
mean less cluster-internal traffic and thus can result in faster execution. Thus the
optimization is only applied in the cluster. As an extra benefit, the optimization 
will also make the affected queries eligible for parallel execution.
