...
- **analyzers** (_optional_; type: `array`; subtype: `string`; default: `[
  ...
  A list of Analyzers, by name as defined via the [Analyzers](../../analyzers/),
...
{{% /hints/info %}}
  ...
  See [Nested search with ArangoSearch](arangosearch-nested-search)
  for details.
...
- **inBackground** (_optional_; type: `boolean`; default: `false`)
  ...
  [Creating Indexes in Background](../indexing-index-basics#creating-indexes-in-background)
...
- **cache** (_optional_; type: `boolean`; default: `false`)
  ...
  See the [`--arangosearch.columns-cache-limit` startup option](../../programs-tools/arangodb-server/programs-arangod-options#--arangosearchcolumns-cache-limit)
...
- **primarySort** (_optional_; type: `array`; default: `[]`; _immutable_)
  ...
  Also see [Primary Sort Order](arangosearch-performance#primary-sort-order)
...
- **primarySortCache** (_optional_; type: `boolean`; default: `false`; _immutable_)
  ...
  [primary sort order](arangosearch-performance#primary-sort-order).
  Otherwise, these values are memory-mapped and it is up to the operating system
  to load them from disk into memory and to evict them from memory.
...
- **storedValues** (_optional_; type: `array`; default: `[]`; _immutable_)
  ...
  - The optional `cache` attribute allows you to always cache stored values in
    ...
    [`--arangosearch.columns-cache-limit` startup option](../../programs-tools/arangodb-server/programs-arangod-options#--arangosearchcolumns-cache-limit)
    to control the memory consumption of this cache.
