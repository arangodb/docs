...
Analyzers can be managed via an [HTTP API](.) and through
a [JavaScript module](../appendix/javascript-modules/appendix-java-script-modules-analyzers).
...
Also see:
- [`SEARCH` operation](../aql/high-level-operations/operations-search) on how to query indexed
  values such as numbers and nested values
- [`arangosearch` Views](../indexing/arangosearch/arangosearch-views) and
  [Inverted indexes](../indexing/working-with-indexes/indexing-inverted) for details about how
...
- **frequency**: track how often a term occurs.
  Required for [`PHRASE()`](../aql/functions/functions-arangosearch#phrase),
  [`NGRAM_MATCH()`](../aql/functions/functions-arangosearch#ngram_match),
  [`BM25()`](../aql/functions/functions-arangosearch#bm25),
  [`TFIDF()`](../aql/functions/functions-arangosearch#tfidf), and
...
- **norm**: write the field length normalization factor that is used to score
  repeated terms fairer. Required for [`BM25()`](../aql/functions/functions-arangosearch#bm25)
  (except BM15) and [`TFIDF()`](../aql/functions/functions-arangosearch#tfidf)
  (if called with normalization enabled).
- **position**: enumerate the tokens for position-dependent queries. Required
  for [`PHRASE()`](../aql/functions/functions-arangosearch#phrase),
  [`NGRAM_MATCH()`](../aql/functions/functions-arangosearch#ngram_match), and
  [`OFFSET_INFO()`](../aql/functions/functions-arangosearch#offset_info).
...
- **offset**: enable search highlighting capabilities (Enterprise Edition only).
  Required for [`OFFSET_INFO()`](../aql/functions/functions-arangosearch#offset_info).
...
- `returnType` (string): data type of the returned tokens. If the indicated
  ...
  applied (see [TO_STRING()](../aql/functions/functions-type-cast#to_string),
  [TO_NUMBER()](../aql/functions/functions-type-cast#to_number),
  [TO_BOOL()](../aql/functions/functions-type-cast#to_bool))
