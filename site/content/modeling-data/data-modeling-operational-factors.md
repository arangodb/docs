...
- Indexes with [indexed array values](../indexing/indexing-index-basics#indexing-array-values)
...
- Sparse indexes can be smaller than non-sparse indexes, but they can only be
  ...
  (also see [Type and value order](../aql/aql-fundamentals/fundamentals-type-value-order)).
...
Use the [query profiler](../aql/execution-and-performance/execution-and-performance-query-profiler)
...
[Index Basics](../indexing/indexing-index-basics).
...
[split edge collections](../graphs/#multiple-edge-collections-vs-filters-on-edge-document-attributes)
...
for more information see [SmartGraphs](../graphs/smartgraphs/)
...
- Transactions are held in-memory before they are committed.
  ...
  [limitations section](../transactions/transactions-limitations#rocksdb-storage-engine).
