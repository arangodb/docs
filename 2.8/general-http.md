...
option ["--scheduler.maximal-queue-size"](configure-arango-communication.html),
...
the server must be started with the option [--server.disable-authentication](configure-arango-arangod.html). 
...
There is an additional option [-server.authenticate-system-only](configure-arango-arangod.html)
