...
**Note:** For this recipe you need at least ArangoDB 2.5. For ArangoDB 2.4 look at the [old Foxx Authentication](foxx-auth-legacy.html).

## Solution

In order to introduce authentication to the todo app we [built](foxx-first-steps.html), we first need to create a route to register users. In order to do that, we first introduce a new Foxx.Model *Credentials* with `username` and `password`:
