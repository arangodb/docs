...
option ["--scheduler.maximal-queue-size"](configurearango-communication.html),
...
the server must be started with the option [--server.disable-authentication](configurearango-arangod.html). 
...
There is an additional option [-server.authenticate-system-only](configurearango-arangod.html)
