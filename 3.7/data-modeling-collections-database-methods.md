...
- *keyOptions* (optional): additional options for key generation. If
  ...
  - *type*: specifies the type of the key generator. The currently
    ...
    *padded*.
    ...
    server deployments. The reason is that document keys can be generated by the 
    DB server, but also by coordinators (of which there are normally multiple
    instances). While each component will still generate an ascending sequence
    of keys, the overall sequence (mixing the results from different components)
    may not be ascending. ArangoDB version 3.10 will change this behavior so that
