---
layout: default
description: This manual describes the ArangoDB importer arangoimp, which can be used forbulk imports
---
Arangoimp
=========
...
It is possible to import [document keys](appendix-glossary.html#document-key) with the documents using the *_key*
attribute. When importing into an [edge collection](appendix-glossary.html#edge-collection), it is mandatory that all
...
a file. The first format that we also used above is commonly known as [jsonl](http://jsonlines.org){:target="_blank"}).
