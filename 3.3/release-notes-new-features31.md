...
graph operations.
For more detailed information read [this manual section](graphs-smart-graphs.html).
...
Audit logging has been added, see [Auditing](administration-auditing.html).
...
It is now easy to get started with providing GraphQL APIs in Foxx, see [Foxx GraphQL](foxx-graph-ql.html).

### OAuth2

Foxx now officially provides a module for implementing OAuth2 clients, see [Foxx OAuth2](foxx-oauth2.html).

### Per-route middleware

It's now possible to specify middleware functions for a route when defining a route handler. These middleware functions only apply to the single route and share the route's parameter definitions. Check out the [Foxx Router documentation](foxx-router.html) for more information.
