...
* added [LDAP](administration-configuration-ldap.html) authentication (Enterprise only)
...
Read more in the [overview](administration-managingusers.html).


Foxx and authorization
----------------------

* the [cookie session transport](foxx-sessions-transports-cookie.html) now
  supports all options supported by the [cookie method of the response object](foxx-router-response.html#cookie).

* it's now possible to provide your own version of the `graphql-sync` module when using the [GraphQL extensions for Foxx](foxx-graphql.html) by passing a copy of the module using the new _graphql_ option.

* custom API endpoints can now be tagged using the [tag method](foxx-router-endpoints.html#tag) to generate a cleaner Swagger documentation.
