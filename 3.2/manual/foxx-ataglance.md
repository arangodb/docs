---
layout: default
description: Each Foxx service is defined by a JSON manifest specifying theentry point, any scripts defined by the service, possible configuration optionsand Foxx dependencies, as well as other metadata
---
Foxx at a glance
================

Each Foxx service is defined by a [JSON manifest](foxx-manifest.html) specifying the
...
options are exposed as the [service context](foxx-context.html).

At the heart of the Foxx framework lies the [Foxx Router](foxx-router.html)
...
[ArangoDB database API](foxx-modules.html).
...
[Scripts](foxx-scripts.html) can be used to perform one-off tasks, which can also be scheduled to be performed asynchronously using the built-in job queue.
