...
const schemaLoader = require("../custom/schema");
...
class Controller {
  ...
  */
  ...
  static getResponseBodyName(request, code) {
    const refObjectPath = request.openapi.schema.responses[code].content['application/json'].schema.$ref;
    if (refObjectPath !== undefined && refObjectPath.length > 0) {
      return (refObjectPath.substr(refObjectPath.lastIndexOf('/') + 1));
    }
    return 'body';
  ...
  static createResponse(request, responseStatus) {
    var responseBodyName = this.getResponseBodyName(request, responseStatus.toString());
    var responseBody = schemaLoader.getComponent(responseBodyName).example;
    return new Response(serviceResponse.code, responseBody, null)
  }

  static async handleRequest(request, response, serviceOperation) {
    try {
      console.log("Handle Request");
      var serviceResponse = await serviceOperation(this.collectRequestParams(request));
      const res = createResponse(request, serviceResponse.code);
